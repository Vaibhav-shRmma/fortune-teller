...
export default function Home() {
  ...
  async function handleFortune() {
    if (!name || !dob) return setError('Name and birth date are required')
    setStep('loading')
    setError('')
    try {
      const res = await fetch('/api/fortune', {
        ...
        body: JSON.stringify({ txSig, name, dob, time, city }),
      })
  ...
  return (
    <main style={{ maxWidth: 480, margin: '60px auto', padding: '0 20px' }}>
      <h1 style={{ fontSize: 22, marginBottom: 8 }}>🔮 Fortune Teller</h1>
      <p style={{ color: '#888', fontSize: 13, marginBottom: 32 }}>
        Pay 1 SOL (devnet) · Get your AI-generated fortune
      </p>
      ...
      {step === 'connect' && (
        <div>
          ...
          <WalletMultiButton />
          {publicKey && (
            <button onClick={() => setStep('pay')} style={btnStyle}>
              Continue →
            </button>
          )}
        </div>
      ...
      {step === 'pay' && (
        <div>
          ...
          <p style={{ fontSize: 14, marginBottom: 20 }}>
            Send <strong>1 SOL</strong> (devnet) to unlock your fortune.
          </p>
          <button onClick={handlePay} style={btnStyle}>Pay 1 SOL</button>
          {error && <p style={{ color: '#f66', marginTop: 12, fontSize: 13 }}>{error}</p>}
        </div>
      )}

      {step === 'form' && (
        <div>
          <p style={{ fontSize: 13, color: '#4a4', marginBottom: 20 }}>
            ✓ Payment confirmed. Enter your birth details.
          </p>
          ...
          <label style={labelStyle}>Birth city (optional)</label>
          <input style={inputStyle} value={city} onChange={e => setCity(e.target.value)} placeholder="e.g. Mumbai" />

          <button onClick={handleFortune} style={btnStyle}>Reveal My Fortune →</button>
          {error && <p style={{ color: '#f66', marginTop: 12, fontSize: 13 }}>{error}</p>}
        </div>
      )}

      {step === 'loading' && (
        <p style={{ color: '#aaa', fontSize: 14 }}>The stars are aligning...</p>
      )}

      {step === 'result' && (
        <div>
          <p style={{ fontSize: 13, color: '#888', marginBottom: 12 }}>Your fortune:</p>
          <div style={{
            background: '#1a1a1a',
          ...
          <button
            onClick={() => { setStep('connect'); setFortune(''); setTxSig('') }}
